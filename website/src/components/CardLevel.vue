...
export default {
	...
	computed: {
		creators() {
			if (this.item.creators && this.item.creators.length > 0) return this.item.creators.join(', ');
	...
	<div class="level-card" :style="{ 'background-color': cardColor }">
		<div class="card" @mouseover="imageHovered = true" @mouseleave="imageHovered = false">
			<a class="card-images" target="_blank" :href="viewerURL" @click="setListIndex(index)">
				...
				<img
					v-if="hasImage && !isModerationCell"
					class="thumbnail"
					loading="lazy"
					:src="this.$images_server_url + this.images.thumb.key"
					:width="this.images.thumb.width"
					:height="this.images.thumb.height"
					@error="handleThumbnailError()"
					@load="this.thumbLoaded = true"
				/>
				<div v-if="hasImage && isModerationCell" class="moderation-images">
					<img
						v-for="(image, i) in this.imageKeys"
						v-show="i == this.currentModerationImage"
						class="thumbnail"
						loading="lazy"
						:src="image"
						:key="image"
			...
			<div class="card-hover">
				<div class="hover-top">
					<ThumbnailFullscreenButton
						v-if="hasImage && !isModerationCell"
						:imageUrl="this.$images_server_url + this.images.full.key"
						:thumbnailUrl="this.$images_server_url + this.images.thumb.key"
					/>
		...
		<div v-if="!listType.startsWith('curated_')" class="privileged-buttons">
			...
			<HideLevelButton
				...
				v-if="
					...
					isSuperModerator &&
					!isModerationCell &&
					this.listType !== 'tab_verify_queue' &&
					this.listType !== 'tab_deletion_queue'
				"
				:level_id="item.identifier"
				@hide="hideState"
			/>
			<UnhideLevelButton
				v-show="isHidden"
				v-if="isSuperModerator && !isModerationCell && this.listType !== 'tab_verify_queue'"
				:level_id="item.identifier"
				@hide="hideState"
			/>

			<UnscheduleDeletionButton
				v-if="!isRecovered && isSuperModerator && this.listType === 'tab_deletion_queue'"
				:level_id="item.identifier"
				@recovered="isRecovered = true"
			/>
		</div>
		<ReportModerationTools
			v-if="isModerationCell"
			:moderation-item="moderationItem"
			@hide="isHidden = true"
...
.play-button:hover,
.card:has(> .card-images:hover) > .card-hover > .hover-top > .play-button {
