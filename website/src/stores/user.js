...
export const useUserStore = defineStore('user', {
	...
	getters: {
		...
		isModerator: (state) => {
			if (!state.isLoggedIn) return false;
			return state.user.info.is_moderator === true || state.user.info.is_supermoderator === true || state.user.info.is_admin === true;
		},
		isVerifier: (state) => {
			if (!state.isLoggedIn) return false;
			return (
				state.user.info.is_verifier === true ||
	...
	actions: {
		async fetchUser(serverURL, authInfo) {
			if (authInfo !== null && 'org_scoped_id' in authInfo && 'code' in authInfo) {
				let response = await fetch(
					serverURL +
						'get_access_token?service_type=oculus_web_demo&service_user_token=' +
						authInfo.org_scoped_id +
						':' +
						authInfo.code,
				);
		...
		getProcessedList(listType) {
			//reported levels, reported users just so it doesnt reshow
