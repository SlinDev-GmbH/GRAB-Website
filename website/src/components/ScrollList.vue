	...
	methods: {
		...
		itemMatchesFilter(item) {
			if (this.listType == 'tab_audit') {
				return (
					(this.auditFilter.user_name === 'User' || item.userInfo?.user_name == this.auditFilter.user_name) &&
...
<template>
	...
	<DropDown
		v-if="listType == 'tab_reported_levels' && isSuperModerator"
		:options="['All', 'Sexual', 'Tips', 'Violence', 'Hatespeech', 'Glitch', 'Loweffort', 'Other']"
		:defaultChoice="'All'"
		:flip="true"
		@changeSelection="filterChoice = $event"
		style="margin-bottom: 5px"
	/>
	<DropDown
		v-if="listType == 'tab_audit' && isSuperModerator"
		:options="['User', ...new Set(items.map((e) => e.userInfo?.user_name))]"
		:defaultChoice="'User'"
		:flip="true"
		@changeSelection="auditFilter.user_name = $event"
		style="margin-bottom: 5px"
	/>
	<DropDown
		v-if="listType == 'tab_audit' && isSuperModerator"
		:options="['Action', ...new Set(items.map((e) => e.request.split(/\/|\?/)[5]))]"
