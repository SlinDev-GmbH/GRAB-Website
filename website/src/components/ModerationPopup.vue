	...
	computed: {
		options() {
			if (this.config.includes('level_')) {
				let reasons = [
					{ reason: 'level_sexual', title: 'Sexual Content / Genitals' },
				...
				return [
					{ reason: 'user_hatespeech', title: 'Inappropriate Language' },
		...
		<div v-if="show" class="modal-mask">
			<div class="modal-container">
				...
				<div class="modal-body">
					<DropDown
						v-if="config != 'user_message'"
						:options="['Select', ...options.map((o) => o.title)]"
						:defaultChoice="currentSelection"
						:flip="true"
						@changeSelection="currentSelection = $event"
					/>
					<form>
						<textarea
							v-if="config === 'level_hide' || config === 'user_message'"
							v-model="message"
							id="message"
							:placeholder="config === 'user_message' ? 'Message' : 'Optional Message'"
							cols="30"
						></textarea>
						<input
							v-if="config === 'user_ban'"
							v-model="banDuration"
							type="number"
							style="width: 70%"
