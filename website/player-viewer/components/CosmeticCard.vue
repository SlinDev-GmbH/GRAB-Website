	...
	methods: {
		isEquipped(side = null) {
			...
			if (side) {
				...
				return Object.values(activeModels).some((model) => (model ? model.name === this.itemName : false));
			}
		},
...
<template>
	<div
		v-show="
			fileExists &&
			(itemObject.type === filterType || (filterType === 'All' && !itemObject.type.includes('currency'))) &&
			(isEquipped || !itemName.includes('rotation'))
		"
		class="cosmetic-card"
	>
